...
public class LtStringTimestampFunctionFactory extends AbstractBooleanFunctionFactory implements FunctionFactory {
    @Override
    ...
    public Function newInstance(ObjList<Function> args, int position, CairoConfiguration configuration) throws SqlException {
        Function strFn = args.getQuick(0);
        if (strFn.isConstant()) {
            try {
                long value = TableUtils.toTimestampOrException(strFn.getStr(null));
                return new LtTimestampVVFunction(position, value, args.getQuick(1), isNegated);
    ...
    private static class LtTimestampStringFunction extends BooleanFunction implements BinaryFunction {
        ...
        public boolean getBool(Record rec) {
            long value;
            final CharSequence str = left.getStr(rec);
            try {
            ...
            return isNegated == (value >= right.getTimestamp(rec));
        }
    ...
    private static class LtTimestampVVFunction extends BooleanFunction implements UnaryFunction {
        ...
        public LtTimestampVVFunction(int position, long left, Function right, boolean isNegated) {
            super(position);
            this.left = left;
            this.right = right;
            this.isNegated = isNegated;
        ...
        @Override
        public boolean getBool(Record rec) {
            return isNegated == (left >= right.getTimestamp(rec));
        }
    }
}
